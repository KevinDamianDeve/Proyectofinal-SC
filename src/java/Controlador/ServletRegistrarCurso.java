...
public class ServletRegistrarCurso extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String CAR= request.getParameter("curso") ;
         String est= request.getParameter("esta") ;
        CURSO f = new CURSO();
        f.setNombrecurso(CAR);
        f.setEstado(est);
        if (CURSO.RegistrarCurso(f)) {
            
          request.setAttribute("mensaje","EL CURSO SE INGRESO DE MANERA CORRECTA");
        }else{
            request.setAttribute("mensaje","ERROR EL CURSO NO SE INGRESO");
        }
